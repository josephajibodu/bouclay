...
import { index, store } from '@/routes/subscriptions';
import type {
    CollectionMode,
...
export default function SubscriptionsCreate({
    ...
    const { transform, post, processing, errors } = useForm<FormShape>({
    ...
    return (
        <div className="mx-auto grid max-w-6xl grid-cols-1 gap-8 p-4 lg:grid-cols-[1fr_360px]">
                ...
                <section className="space-y-3 rounded-lg border p-4">
                    ...
                    {lines.length === 0 ? (
                        <p className="text-sm text-muted-foreground">
                            Add the product or plan you're billing for. Add a
                            free trial if you offer one.
                        </p>
                    ) : (
                    ...
                    {customerId !== null && (
                        <div className="flex flex-wrap gap-3">
                            <div className="w-52">
                                <Select value="" onValueChange={addPrice}>
                                    ...
                                    <SelectContent>
                                        {productOptions.length === 0 ? (
                                            <div className="p-2 text-xs text-muted-foreground">
                                                No recurring prices in {currency}
                                                .
                                            </div>
                                        ) : (
                            ...
                            {trialOptions.length > 0 && (
                                <div className="w-52">
                                    <Select value="" onValueChange={addTrial}>
                                        <SelectTrigger data-test="add-trial">
                                            <span className="flex items-center gap-1.5 text-sm">
                                                <Gift className="size-4" /> Add
                                                trial
                                            </span>
                                        </SelectTrigger>
                                        <SelectContent>
                                            {trialOptions.map((offer) => (
                                                <SelectItem
                                                    key={offer.id}
                        ...
                        <div className="space-y-2">
                            {customer.paymentMethods.length > 0 ? (
                                ...
                                <p className="rounded-md bg-amber-50 p-3 text-xs text-amber-700 dark:bg-amber-950/40 dark:text-amber-400">
                                    {customer.name ?? 'This customer'} doesn't
                                    have a card on file. We'll create the
                                    subscription and send a secure checkout link
                                    to collect one. Access starts once they pay.
                                </p>
                            )}
                        </div>
                    )}
